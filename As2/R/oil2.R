...
save(oil.g11.fit, file="oil.g11.fit")
save(oil.g11f, file="oil.g11f")
load("oil.ag11f")
...
for(i in seq(1,1250,1)){
  ...
  oil.fit.s11<-as.numeric(unlist(oil.g11f[[i]]@forecast[["seriesFor"]]))
  write.table(oil.fit.s11, file="oilsgarchfit.txt",
              append=TRUE,
              col.names = FALSE,
              sep = ',',row.names=FALSE)
  
  oil.vol.s11<-as.numeric(unlist(oil.g11f[[i]]@forecast[["sigmaFor"]]))
...
for(i in seq(1,1250,1)){
  ...
  write.table(oil.vol.gjrg11, file="oilgjrgarchvol.txt",
              ...
              col.names = FALSE,
              sep = ',',row.names=FALSE)
  print(oil.fit.gjrg11)
  print(oil.vol.gjrg11)
}
#########################################################################################################
#########################################################################################################
#########################################################################################################
...
##or??????(Result exactly same)
oilols2=list()
oilolsf2=vector()
for(i in 1:1250){
  oilols2[[i]]=ar(oil.v[i:(3183+i)], aic = TRUE, order.max = 1, na.action = na.fail,
                 demean = FALSE, intercept = TRUE, method = "ols")
  oilolsf2[[i]]<-forecast(oilols2[[i]],h=1)$mean
...
out.forc[[i]]=capture.output(forc[[i]][[1]][[1]])
cat(out.forc[[i]],file="ar1.forc.oilf.rets.txt",sep="\n",append=TRUE)
